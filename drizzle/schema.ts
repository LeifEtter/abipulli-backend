import { pgTable, unique, integer, varchar, foreignKey, bigint, text, timestamp, smallint, index, boolean } from "drizzle-orm/pg-core"
...
export const imageToDesign = pgTable("image_to_design", {
...
export const designSuggestion = pgTable("design_suggestion", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	id: bigint({ mode: "number" }).notNull(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	chatId: bigint("chat_id", { mode: "number" }),
...
export const image = pgTable("image", {
	...
	index("image_index_1").using("btree", table.generated.asc().nullsLast().op("bool_ops")),
	foreignKey({
			columns: [table.orderId],
			foreignColumns: [order.id],
